...
Player::Player(const std::shared_ptr<Executor>& ui_executor)
{
...
auto Player::CreateInstance(const std::shared_ptr<Executor>& ui_executor) -> std::shared_ptr<Player>
{
    return std::shared_ptr<Player>{ new Player(ui_executor) };
}
...
auto Player::DecodeAudioThread() -> void
{
    while (true) {
        ...
        if (receive_frame_err == DecodeErrors::Ok) {
            AVRational time_base = audio_decoder_->GetPacketTimeBase();
            // TODO(Light Lin):
...
auto Player::DecodeVideoThread() -> void
{
    ...
    while (true) {
        ...
        if (receive_frame_err == DecodeErrors::Ok) {
            ...
            auto pts = (frame.RawFramePtr()->pts == AV_NOPTS_VALUE) ? NAN : frame.RawFramePtr()->pts * av_q2d(video_decoder_->GetPacketTimeBase());
...
auto Player::VideoFrameDuration(const Frame& frame, const Frame& next_frame) const -> double
{
...
auto Player::ComputeVideoFrameTargetDelay(double delay) const -> double
{

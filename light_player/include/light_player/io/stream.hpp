...
class LIGHT_PLAYER_API IStream : public RefCounted {
 public:
  virtual ~IStream() noexcept {}
  virtual auto CanSeek() const noexcept -> bool = 0;
  virtual auto Read(std::uint8_t *buf, std::size_t buf_size) noexcept
      -> int = 0;
  virtual auto Seek(std::int64_t offset, int whence) noexcept
      -> std::int64_t = 0;
};

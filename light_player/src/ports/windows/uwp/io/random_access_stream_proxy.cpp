...
RandomAccessStreamProxy::RandomAccessStreamProxy(IRandomAccessStream ras) {
  HRESULT hr = CreateStreamOverRandomAccessStream(
      winrt::get_unknown(ras), __uuidof(::IStream), stream_.put_void());
  if (FAILED(hr)) {
    winrt::throw_hresult(hr);
  }
}

auto RandomAccessStreamProxy::AddRef() -> std::int64_t { return ++ref_count_; }
...
auto RandomAccessStreamProxy::CanSeek() const noexcept -> bool { return true; }

auto RandomAccessStreamProxy::Read(std::uint8_t *buf,
                                   std::size_t buf_size) noexcept -> int {
  ULONG bytesRead{0};
...
auto RandomAccessStreamProxy::Seek(std::int64_t offset, int whence) noexcept
    -> std::int64_t {
  LARGE_INTEGER in{0};
  in.QuadPart = offset;

...
namespace lp::ports::windows::uwp::renderer {
	...
	{
		VideoFrameData(std::uint32_t width, std::uint32_t height, IBuffer y_data, IBuffer u_data, IBuffer v_data, std::uint32_t y_stride, std::uint32_t u_stride, std::uint32_t v_stride)
			: width(width)
			, height(height)
			, y_data(y_data)
			, u_data(u_data)
	...
	class SwapChainPanelRenderer : public std::enable_shared_from_this<SwapChainPanelRenderer>
	{
	public:
		...
		auto RenderYUV420P(std::uint32_t width, std::uint32_t height, IBuffer ydata, IBuffer udata, IBuffer vdata, std::uint32_t y_stride, std::uint32_t uv_stride) -> void;
		auto DoRenderYUV420P(std::uint32_t width, std::uint32_t height, IBuffer ydata, IBuffer udata, IBuffer vdata, std::uint32_t y_stride, std::uint32_t uv_stride) -> void;
		auto TrimAndRelease() -> void;

	private:
		auto CreateTexture2D(std::uint32_t width, std::uint32_t height) -> winrt::com_ptr<ID3D11Texture2D>;
		auto CreateShaderResourceView(ID3D11Resource* resource) -> winrt::com_ptr<ID3D11ShaderResourceView>;

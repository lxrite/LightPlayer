...
auto UWPRenderImpl::AddRef() -> std::int64_t { return ++ref_count_; }
...
auto UWPRenderImpl::Render(const Picture &picture) noexcept -> void {
  ...
  if (picture.format() == AV_PIX_FMT_YUV420P) {
    ...
    const std::uint8_t *y_data =
        reinterpret_cast<const std::uint8_t *>(yuv_data[0]);
    const std::uint8_t *u_data =
        reinterpret_cast<const std::uint8_t *>(yuv_data[1]);
    const std::uint8_t *v_data =
        reinterpret_cast<const std::uint8_t *>(yuv_data[2]);
    ...
    renderer_->RenderYUV420P(width, height, y_buffer, u_buffer, v_buffer,
                             y_linesize, uv_linesize);
  } else {
    auto target_pix_fmt = AV_PIX_FMT_YUV420P;
    int align = 8;
    auto output_buffer_size_need =
        av_image_get_buffer_size(target_pix_fmt, width, height, align);
    if (scale_output_buffer_ == nullptr ||
    ...
    int linesize[8] = {0};
    av_image_fill_arrays(data, linesize, scale_output_buffer_.get(),
                         target_pix_fmt, width, height, align);
    const auto &picture_data = picture.data();
    img_convert_ctx_ = sws_getCachedContext(
        img_convert_ctx_, width, height,
        static_cast<AVPixelFormat>(picture.format()), width, height,
        target_pix_fmt, SWS_BICUBIC, nullptr, nullptr, nullptr);
    sws_scale(img_convert_ctx_, picture.data(), picture.linesize(), 0, height,
              data, linesize);
...
auto UWPRenderImpl::MakeBuffer(const std::uint8_t *data, size_t size)
    -> IBuffer {

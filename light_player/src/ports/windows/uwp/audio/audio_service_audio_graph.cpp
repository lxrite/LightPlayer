...
AudioService_AudioGraph::AudioService_AudioGraph(
    AudioGraph graph, AudioFrameInputNode input_node,
  ...
  input_node_.AudioFrameCompleted(
      [this, audio_frame_completed](AudioFrameInputNode node,
                                    AudioFrameCompletedEventArgs e) {
...
auto AudioService_AudioGraph::Start() -> void { audio_graph_.Start(); }

auto AudioService_AudioGraph::Stop() -> void { audio_graph_.Stop(); }

auto AudioService_AudioGraph::WriteAudioFrameData(std::uint8_t *data,
  ...
  buffer_ref.as<IMemoryBufferByteAccess>()->GetBuffer(&write_buf,
                                                      &write_buf_size);
  std::memcpy(write_buf, data, size);
  buffer_ref.Close();
  buffer.Close();
  // A maximum of 64 frames can be queued in the AudioFrameInputNode at one
  // time. Attempting to queue more than 64 frames will result in an error.
  ++queued_freme_count_;
...
auto AudioService_AudioGraph::CreatePCMAudioService(
  ...
  auto aep = AudioEncodingProperties::CreatePcm(sample_rate, channel_count,
                                                bits_per_sample);
  ...
  auto createOutputNodeResult =
      co_await audioGraph.CreateDeviceOutputNodeAsync();
  if (createOutputNodeResult.Status() !=
  ...
  co_return std::make_shared<AudioService_AudioGraph>(
      audioGraph, frameInputNode, deviceOutputNode, audio_frame_completed);
}

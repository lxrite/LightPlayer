...
namespace lp::ports::windows::uwp::audio {
	...
	class AudioService_AudioGraph
		: public AudioService {
	public:
		AudioService_AudioGraph(AudioGraph graph, AudioFrameInputNode input_node, AudioDeviceOutputNode output_node, std::function<void()> audio_frame_completed);
		...
		static auto CreatePCMAudioService(std::uint32_t sample_rate, std::uint32_t channel_count, std::uint32_t bits_per_sample, std::function<void()> audio_frame_completed) -> Task<std::shared_ptr<AudioService_AudioGraph>>;

...
static auto SeekCallback(void* opaque, std::int64_t offset, int whence) -> std::int64_t
{
	auto stream = reinterpret_cast<lp::io::IStream*>(opaque);
	return stream->Seek(offset, whence);
}

CustomIOWrapper::CustomIOWrapper(IStream *stream)
	: stream_(stream, true)
{
	...
	AVIOContext *io_ctx = avio_alloc_context(io_buffer, buffer_size, 0, stream_.get(), &ReadCallback, nullptr, seek_callback);
	if (io_ctx == nullptr) {
		// need free io_buffer?
...
auto CustomIOWrapper::IOContext() const -> AVIOContext*
{
	return io_ctx_;
}

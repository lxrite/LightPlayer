...
class PlayerImpl : public std::enable_shared_from_this<PlayerImpl> {
 ...
 private:
  ...
  auto OpenAsync(const std::string url,
                 const std::shared_ptr<io::CustomIOWrapper> custom_io)
      -> FireAndForget;
  auto DoOpen(const std::string &url,
              const std::shared_ptr<io::CustomIOWrapper> &custom_io)
      -> Task<void>;
 ...
 private:
  auto VideoFrameDuration(const Frame &frame, const Frame &next_frame) const
      -> double;
  ...
  auto SwitchToUIThread() {
    struct Awaitable {
      ...
      void await_suspend(std::coroutine_handle<> handle) const noexcept {
        ui_executor->Dispatch([handle]() { handle.resume(); });
      }

...
#include "include/light_player/player.hpp"
...
Player::Player(Executor *ui_executor) {
  impl_ = reinterpret_cast<void *>(new std::shared_ptr<PlayerImpl>(
      std::make_unique<PlayerImpl>(ui_executor)));
}
...
auto Player::OpenStream(io::IStream *stream) -> PlayerOperationResult {
  return (*reinterpret_cast<std::shared_ptr<PlayerImpl> *>(impl_))
      ->OpenStream(stream);
}
...
auto Player::SetRenderer(Renderer *renderer) -> void {
  (*reinterpret_cast<std::shared_ptr<PlayerImpl> *>(impl_))

...
auto PlayerImpl::GetDuration() const noexcept -> std::int64_t {
  auto state = GetState();
  if (state == PlayerState::Opended || state == PlayerState::Playing ||
...
auto PlayerImpl::Open(const std::string &url,
                      const std::shared_ptr<io::CustomIOWrapper> &custom_io)
    -> lp::PlayerOperationResult {
  if (player_state_ != PlayerState::Ready) {
    return PlayerOperationResult::Fail;
  }
...
auto PlayerImpl::OpenAsync(const std::string url,
                           const std::shared_ptr<io::CustomIOWrapper> custom_io)
    -> FireAndForget {
  ++running_task_cnt_;
...
auto PlayerImpl::DoOpen(const std::string &url,
                        const std::shared_ptr<io::CustomIOWrapper> &custom_io)
    -> Task<void> {
  ...
  do {
    ...
    audio_stream_index_ = av_find_best_stream(format_ctx_, AVMEDIA_TYPE_AUDIO,
                                              -1, -1, nullptr, 0);
    video_stream_index_ = av_find_best_stream(format_ctx_, AVMEDIA_TYPE_VIDEO,
                                              -1, -1, nullptr, 0);
    ...
    if (audio_stream_index_ >= 0) {
      ...
      if (audio_codec != nullptr) {
        audio_decoder_ =
            Decoder::OpenDecoder(audio_codecpar, audio_codec, nullptr);
      }
    }
    ...
    if (video_stream_index_ >= 0) {
      ...
      if (video_codec != nullptr) {
        video_decoder_ =
            Decoder::OpenDecoder(video_codecpar, video_codec, nullptr);
      }
    ...
    max_frame_duration_ =
        (format_ctx_->iformat->flags & AVFMT_TS_DISCONT) ? 10.0 : 3600.0;
...
auto PlayerImpl::ReadTask() -> Task<void> {
  ...
  while (true) {
    ...
    if (is_seek_requested) {
      ...
      int64_t seek_target =
          static_cast<std::int64_t>(static_cast<double>(seek_target_us) *
                                    static_cast<double>(seek_tb::num) /
                                    static_cast<double>(seek_tb::den));
      int64_t seek_min = INT64_MIN;
      int64_t seek_max = INT64_MAX;
      int seek_ret = avformat_seek_file(format_ctx_, -1, seek_min, seek_target,
                                        seek_max, 0);
      if (seek_ret < 0) {
        // TODO(Light Lin):
      } else {
      ...
      }
      {
        ...
        co_await read_pkt_cv_.Wait(lck, [this]() -> bool {
          return IsCloseRequested(false) ||
                 (video_stream_index_ >= 0 && video_pkt_queue_.Size() < 10) ||
                 (audio_stream_index_ >= 0 && audio_pkt_queue_.Size() < 10);
        });
      }
...
auto PlayerImpl::DecodeAudioTask() -> Task<void> {
  ...
  while (true) {
    ...
    if (receive_frame_err == DecodeErrors::Ok) {
      // TODO(Light Lin):
      auto duration = av_q2d(AVRational{frame.RawFramePtr()->nb_samples,
                                        frame.RawFramePtr()->sample_rate});
      auto pts = (frame.RawFramePtr()->pts == AV_NOPTS_VALUE)
                     ? NAN
                     : frame.RawFramePtr()->pts * av_q2d(time_base);
    ...
    } else if (receive_frame_err == DecodeErrors::Again) {
      {
        ...
        if (packet.IsFlushPacket() || audio_decoder_->PacketSerial().value() !=
                                          audio_pkt_queue_.Serial()) {
          continue;
        }
...
auto PlayerImpl::DecodeVideoTask() -> Task<void> {
  ...
  AVRational frame_rate = av_guess_frame_rate(
      format_ctx_, format_ctx_->streams[video_stream_index_], nullptr);
  ...
  while (true) {
    ...
    if (receive_frame_err == DecodeErrors::Ok) {
      ...
      if (packet.IsFlushPacket() ||
          video_decoder_->PacketSerial().value() != video_pkt_queue_.Serial()) {
        continue;
      }
  ...
  while (true) {
    {
      ...
      co_await video_render_cv_.Wait(lck, [this]() {
        return IsCloseRequested(false) ||
               (!paused_ && !video_frame_queue_.Empty());
      });
      if (IsCloseRequested(false)) {
        co_return;
      }
    ...
    std::optional<int> packet_queue_serial;
    {
      ...
      if (frame.has_value()) {
        packet_queue_serial =
            std::make_optional<int>(video_pkt_queue_.Serial());
      }
    }
    if (!frame) {
      assert(false);
    ...
    auto last_duration =
        VideoFrameDuration(last_show_frame.value(), frame.value());
    ...
    auto wait_duration = std::chrono::milliseconds(
        static_cast<std::int64_t>(remaining_time * 1000));
...
auto PlayerImpl::AudioTask() -> Task<void> {
  ...
  auto audio_service = co_await lp::audio::CreatePCMAudioService(
      sample_rate, channel_count, bits_per_sample, on_drained);
  ...
  do {
    ...
    auto alloc_ctx_result = swr_alloc_set_opts2(
        &au_convert_ctx, &out_channel_layout, out_sample_fmt, out_samples_rate,
    ...
    while (true) {
      {
        ...
        co_await audio_output_cv_.Wait(lck, [this, audio_service]() {
          return IsCloseRequested(false) ||
                 (!paused_ && audio_service->QueuedFrameCount() < 5);
        });
        if (IsCloseRequested(false)) {
          break;
        }
      ...
      if (!frame) {
        std::this_thread::sleep_for(std::chrono::milliseconds(
            static_cast<std::int64_t>(kLightPlayerRefreshRate * 100)));
        continue;
      }
      if (!std::isnan(frame->Pts())) {
        audio_clock =
            frame->Pts() +
            static_cast<double>(frame->RawFramePtr()->nb_samples) /
                static_cast<double>(frame->RawFramePtr()->sample_rate);
      }
      uint8_t *out_audio_data_buf = nullptr;
      unsigned int out_audio_data_buf_size =
          av_samples_alloc(&out_audio_data_buf, nullptr,
                           frame->RawFramePtr()->ch_layout.nb_channels,
                           frame->RawFramePtr()->nb_samples, out_sample_fmt, 0);
      int out_samples_per_channel = swr_convert(
          au_convert_ctx, &out_audio_data_buf, out_audio_data_buf_size,
          (const std::uint8_t **)(frame->RawFramePtr()->extended_data),
          frame->RawFramePtr()->nb_samples);
      int out_len = (unsigned int)(out_samples_per_channel *
                                   frame->RawFramePtr()->ch_layout.nb_channels *
...
auto PlayerImpl::VideoFrameDuration(const Frame &frame,
                                    const Frame &next_frame) const -> double {
...
auto PlayerImpl::ComputeVideoFrameTargetDelay(double delay) const -> double {
  ...
  if (!std::isnan(diff)) {
    auto sync_threshold = std::clamp(delay, kLightPlayerAVSyncThresholdMin,
                                     kLightPlayerAVSyncThresholdMax);
    if (fabs(diff) < max_frame_duration_) {
      if (diff <= -sync_threshold) {
        delay = FFMAX(0, delay + diff);
      } else if (diff >= sync_threshold &&
                 delay > kLightPlayerAVSyncFramedupThreshold) {
        delay = delay + diff;
      } else if (diff >= sync_threshold) {
        delay = 2 * delay;
      }

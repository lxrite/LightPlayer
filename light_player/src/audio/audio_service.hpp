...
namespace lp::audio {
	...
	public:
		...
		virtual auto WriteAudioFrameData(std::uint8_t* data, std::size_t size) -> void = 0;
		virtual auto QueuedFrameCount() -> std::uint32_t const = 0;
	};

	auto CreatePCMAudioService(std::uint32_t sample_rate, std::uint32_t channel_count, std::uint32_t bits_per_sample, std::function<void()> audio_frame_completed) -> Task<std::shared_ptr<AudioService>>;

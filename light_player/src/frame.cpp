...
auto RawFrameHost::RawFramePtr() const -> RawFrameType * { return raw_frame_; }
...
auto Frame::SetSerial(int serial) noexcept -> void { serial_ = serial; }

auto Frame::Serial() const noexcept -> std::optional<int> { return serial_; }

auto Frame::SetPts(double pts) noexcept -> void { pts_ = pts; }

auto Frame::Pts() const noexcept -> double { return pts_; }

auto Frame::SetDuration(double duration) noexcept -> void {
  duration_ = duration;
}

auto Frame::Duration() const noexcept -> double { return duration_; }
